...
void calcular_ula(int32_t, int32_t, int, int, int, int, int, int, int32_t*, int*);
void print_binario(int32_t);

int main() {
    FILE* arq = fopen("entradas.txt", "r");
    if (!arq) puts("Não foi possível abrir o arquivo");
    ...
    while (fscanf(arq, "%s", bits) == 1) {
        ...
        printf("PC = %d\n", PC);
        printf("IR = %s\n", bits);
        calcular_ula(A, B, F0, F1, ENA, ENB, INVA, INC, &s, &vai_um);
        PC++;
    }
}

void calcular_ula(int32_t A, int32_t B, int F0, int F1, int ENA, int ENB, int INVA, int INC, int32_t* s, int* vai_um) {
    ...
    *vai_um = 0; // inicializa vai_um com 0
    ...
    if (F0 == 0 && F1 == 0) { // se F0 == 0 e F1 == 0, habilita AB
        ...
        *s = (int32_t)(soma_completa & 0xFFFFFFFF);
        ...
        então deslocamos 32 bits para a direita sa soma_completa, e encontramos nosso vai_um,
...
void print_binario(int32_t num) {
